# -*- coding: utf-8 -*-
"""
Created on Sat May 21 12:38:11 2022
...
class LitTransformer(LightningModule):
    def __init__(self, num_classes=37, lr=0.001, max_epochs=30, depth=12, embed_dim=128,
...
class LitKWS37(LightningDataModule):
    def __init__(self,path, n_fft=512, 
                 n_mels=128, win_length=None, hop_length=256, batch_size=32, num_workers=2, patch_num=4, class_dict={}, **kwargs):
        super().__init__()
        self.path=path
        self.n_fft=n_fft
        self.n_mels=n_mels
    ...
    def prepare_data(self):
        ...
        self.val_dataset = torchaudio.datasets.SPEECHCOMMANDS(root=self.path,
                                                              download=True,
                                                              subset='validation')
        self.test_dataset = torchaudio.datasets.SPEECHCOMMANDS(root=self.path,
                                                               download=True,
                                                               subset='testing')                                                    
...
def get_args():
    ...
    parser.add_argument('--dataset', default='KWS37', type=str, metavar='N')
...
if __name__ == "__main__":
    ...
    datamodule = LitKWS37(batch_size=64, num_workers=8,
...
model = LitTransformer(num_classes=37, lr=args.lr, epochs=args.max_epochs, 
                           depth=args.depth, embed_dim=128, head=8,
                           patch_dim=patch_dim, seqlen=seqlen,)
idx_to_class = {v: k for k, v in CLASS_TO_IDX.items()}
trainer = Trainer(accelerator=args.accelerator, devices=args.devices,
                      max_epochs=args.max_epochs, precision=32 if args.accelerator == 'gpu' else 32,callbacks=[model_checkpoint])
model.hparams.sample_rate = datamodule.sample_rate
model.hparams.idx_to_class = idx_to_class
trainer.fit(model, datamodule=datamodule)
trainer.test(model, datamodule=datamodule)
   # trainer.save_checkpoint('../transformer/transformer.ckpt')
        
   
model = model.load_from_checkpoint('KWS_Model_best.ckpt')
model.eval()
model_scripted = torch.jit.script(model) # Export to TorchScript
model_scripted.save('model_scripted_final.pt') # Save
